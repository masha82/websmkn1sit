        ...
        <div class="content-wrap">
            <div class="container clearfix">
                <div class="row">
                    ...
                    @endif
                    <div class="col-lg-6">
                        <form class="row" action="{{ route('bidangpendidik.store') }}" method="post"
                              enctype="multipart/form-data">
                            @csrf
                            @method('POST')
                            <div class="col-12 form-group">
                                <label>Nama Bidang Tenaga Pendidik:</label>
                ...
                <div class="row">
                    <h6 class="text-center">Daftar Bidang Tenaga Pendidik</h6>
                    <div>
                        <table class="table table-striped" id="myTable">
                            <thead>
                            <tr>
                                <th>Nama Bidang Tenaga Pendidik</th>
                                <th>Aksi</th>
                            </tr>
                            </thead>
                            <tbody>
    ...
    <script>
        $(document).ready(function () {
            var table = $('#myTable').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('bidangpendidik.data') }}",
                columns: [{
                    data: 'nama_bidang',
                    name: 'nama_bidang'
                    },
            ...
            var del = function (id) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('bidangpendidik.index') }}/" + id,
                            method: "DELETE",
